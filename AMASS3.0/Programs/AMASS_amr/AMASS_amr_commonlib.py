...
def readxlsxorcsv(spath,sfilename) :
    df = pd.DataFrame()
    try:
        df = pd.read_csv(spath + sfilename + ".csv")
    except:
        try:
            df= pd.read_csv(spath + sfilename + ".csv", encoding="windows-1252")
        except:
...
def readxlsorcsv_noheader(spath,sfilename,columnheader) :
    try:
        df = pd.read_csv(spath + sfilename + ".csv",header=None,names=columnheader)
    except:
        try:
            df= pd.read_csv(spath + sfilename + ".csv", encoding="windows-1252",header=None,names=columnheader)
        except:
            stmpfilename = spath + sfilename + "_temp.csv"
            if os.path.exists(stmpfilename):
                os.remove(stmpfilename)
