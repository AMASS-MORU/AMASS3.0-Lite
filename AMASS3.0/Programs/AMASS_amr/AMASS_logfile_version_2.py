...
def ts0(c,logger,over_raw,fspcdate,fadmdate,fdisdate,fmhn,fhhn,bhosp_avi):
    try:
        ...
        log_1_1 = "<br/>Please review the following information carefully before interpreting the AMR surveillance report generated by the AMASS application."
        log_1_2 = "<br/>The microbiology_data file (stored in the same folder as the application file) had:"
        ...
        if bhosp_avi:
            ...
            #hospital data
            log_2_1 = "<br/>The hospital_admission_data file (stored in the same folder as the application file) had:"
...
try:
    ...
    if (ALogL.checkpoint(var_mi_i) or ALogL.checkpoint(var_mi_icsv)):
        try:
            ts(c,var_mi_raw, var_mi_col, marked_var_mi, "Table S7", "List of variable names in your microbiology_data file")
        except Exception as e:
            logger.exception(e) 
            pass
    if (ALogL.checkpoint(var_ho_i) or ALogL.checkpoint(var_ho_icsv)):
        try:
            ts(c,var_ho_raw, var_ho_col, marked_var_ho, "Table S8", "List of variable names in your hospital_admission_data file")
        except Exception as e:
            logger.exception(e)
            pass
    if ALogL.checkpoint(file_dup):
